...
/** Hard-delete ENTRADA con jornada abierta: solo esa fila. */
async function testHardDeleteEntradaAbierta() {
    ...
    const ids = await hardDeleteComoAdmin(db, registro);
    ...
    assert.strictEqual(restantes.length, 0, 'entrada abierta eliminada por completo');
...
/** Hard-delete ENTRADA emparejada elimina también la SALIDA (sin huérfanas). */
async function testHardDeleteEntradaConSalidaEmparejada() {
    ...
    assert.strictEqual(salidas.length, 0, 'no debe quedar SALIDA huérfana');
    ...
    db.close();
    fs.unlinkSync(tmp);
    ok('hard-delete ENTRADA + SALIDA emparejada en cascada');
}

/** Hard-delete SALIDA sola: solo esa fila; ENTRADA queda abierta. */
async function testHardDeleteSalidaSola() {
    ...
    const ids = await hardDeleteComoAdmin(db, registroSalida);
    ...
    db.close();
    fs.unlinkSync(tmp);
    ok('hard-delete SALIDA sola deja ENTRADA abierta');
}
