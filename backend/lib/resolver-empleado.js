/**
 * Resuelve un empleado activo por código o nombre.
 * El kiosco busca por nombre, pero el registro usaba solo código exacto.
 */
...
function resolverEmpleadoDeLista(empleados, dato) {
    ...
    const porCodigo = activos.filter((e) => normalizarPersona(e.codigo) === norm);
    if (porCodigo.length === 1) {
        return { ok: true, empleado: porCodigo[0] };
    }

    const porNombreExacto = activos.filter((e) => {
    ...
    const parciales = activos.filter((e) => {
        const full = normalizarPersona(nombreCompleto(e));
        ...
        return full.includes(norm) || codigo === norm || nombre === norm || apellido === norm;
    });
